...
- Chrome extension has one purpose: right-click one selected image, open Chrome Side Panel, call the configured TrustPic API, and render the returned `interpretation` without adding local detector logic.
- Chrome Web Store package defaults to `https://trustpic-production.up.railway.app`; the privacy policy page is served by the Web app at `/privacy.html`.
